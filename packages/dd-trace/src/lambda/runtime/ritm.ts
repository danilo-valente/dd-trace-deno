 ...
 * ```
 */
function _extractModuleRootAndHandler(fullHandler: string) {
 ...
 * ```
 */
function _extractModuleNameAndHandlerPath(handler: string) {
  const FUNCTION_EXPR = /^([^.]*)\.(.*)$/;
 ...
 */
function _getLambdaFilePath(lambdaStylePath: string) {
...
const registerLambdaHook = () => {
  ...
  if (originalLambdaHandler !== undefined) {
    ...
    Hook([moduleToPatch], (moduleExports, moduleName: string, _) => {
...
export { _extractModuleNameAndHandlerPath, _extractModuleRootAndHandler, _getLambdaFilePath, registerLambdaHook };
